...
describe('json', function() {
  ...
  it('preserves the graph value, if any', function() {
    expect(rw(new Graph<string, string, string, number>().setGraph(1)).graph()).equals(1);
    expect(rw(new Graph<string, string, string, { foo: string }>().setGraph({ foo: 'bar' })).graph()).eqls({ foo: 'bar' });
  ...
  it('preserves nodes', function() {
    ...
    expect(rw(new Graph<string, number>().setNode('a', 1)).node('a')).equals(1);
    expect(rw(new Graph<string, { foo: string }>().setNode('a', { foo: 'bar' })).node('a'))
      .eqls({ foo: 'bar' });
  });
  ...
  it('preserves simple edges', function() {
    ...
    expect(rw(new Graph<string, string, number>().setEdge('a', 'b', 1)).edge('a', 'b')).equals(1);
    expect(rw(new Graph<string, string, { foo: string }>().setEdge('a', 'b', { foo: 'bar' })).edge('a', 'b'))
      .eqls({ foo: 'bar' });
  });

  it('preserves multi-edges', function() {
    const g = new Graph<string, string, number | { foo: string }>({ multigraph: true });
...
function rw<K extends NodeKey,N,E,G>(g: Graph<K,N,E,G>): Graph<K,N,E,G> {
  return read(write(g));
}
